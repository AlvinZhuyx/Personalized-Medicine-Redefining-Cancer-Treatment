# -*- coding: utf-8 -*-
"""
Created on Mon Nov 20 11:57:52 2017
...
for i in range(fold):
    ...
    y_predict_final = pred.copy()
    #if score < 0.9:
    if denom != 0:
        pred = model.predict(xgb.DMatrix(test), ntree_limit=model.best_ntree_limit+80)
        y_predict_final += pred
        
    denom += 1
y_predict_final /= denom


test_solution = pd.read_csv("../data/stage1_solution_filtered.csv", sep = ",")
...
submission = pd.DataFrame(y_predict_final, columns=['class'+str(c+1) for c in range(9)])
submission['ID'] = pid
